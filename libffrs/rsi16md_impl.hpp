...
protected:
    ...
    inline size_t sugiyama(const GFT synd[], GFT a1[], GFT temp5[]) const {
        ...
        r2[ecc_len] = 1;
        size_t r2_len = ecc_len + 1;

        // R1 = ref.P(GF, (synds))
        auto r1 = &temp5[block_len * 3];
        std::copy_n(&synd[0], ecc_len, &r1[0]);
        size_t r1_len = ecc_len;
        ...
        #define vec_copy(a, b) do { std::copy_n(a, block_len, b); b##_len = a##_len; } while(0)

        nttr(r1);
        nttr(r2);
        nttr(a1);
        // nttr(a2);
        for (size_t i = 0; i < ecc_len / 2 && r1_len > ecc_len / 2; ++i) {
            // Q = R2 // R1
            vec_div(r2, r1, q);
    ...
    inline size_t _vec_inv_mod_xn(GFT h[], size_t /*h_len*/, size_t n, GFT a[]) const {
        ...
        // a = P([h.x[0].inv()])
        std::fill_n(&a[0], block_len, GFT{0});
        inttr(h);
        GFT h_0 = h[0];
        nttr(h);
        a[0] = gf.inv(h_0);
        ...
        while (l < n) {
            ...
            // A = A % x**n
            inttr(a);
            std::fill_n(&a[n], block_len - n, GFT{0});
            nttr(a);
    ...
    inline size_t _vec_div(GFT f[], size_t f_len, GFT g[], size_t g_len, GFT q[]) const {
        ...
        inttr(q);
        std::fill_n(&q[q_len], block_len - q_len, GFT{0});
    ...
    inline void nttr(GFT block[]) const {
        gs_butterfly(&_roots_v_block[0], &block[0], block_len, block_len);
    }

    inline void inttr(GFT block[]) const {
        ct_butterfly(&_roots_iv_block[0], &block[0], block_len);
        for (size_t j = 0; j < block_len; ++j)
            block[j] = gf.mul(block[j], block_len_iv);
    }
};
