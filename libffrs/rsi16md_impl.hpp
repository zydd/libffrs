...
#define print_vec(v) \
    if constexpr(std::is_integral_v<GFT>) py::print(#v ":", v##_len, std::vector(&v[0], &v[ecc_len]))
#define print_Vec(v) \
    do { if constexpr(std::is_integral_v<GFT>) { inttr(v); print_vec(v); nttr(v); } } while(0)
#define print_VEC(v) \
    do { \
        py::print("-- VEC " #v " = ["); \
...
public:
     ...
     */
    inline void pintt_ecc(GFT ntt_block[]) const {
    ...
    inline void repair(GFT block[], GFT temp_ntt1_ecc6[]) const {
        ...
        // compute synds
        std::copy_n(&block[0], block_len, &temp_ntt1_ecc6[0]);
        pntt(&temp_ntt1_ecc6[0]);

        // init evaluator_poly with synds
        auto evaluator_poly = &temp_ntt1_ecc6[ecc_len * 0];
        ...
        auto error_locations = &temp_ntt1_ecc6[ecc_len * 2];
        auto error_locations_rbo = &temp_ntt1_ecc6[ecc_len * 3];

        auto roots = &temp_ntt1_ecc6[ecc_len * 4];
...
protected:
    ...
    inline size_t find_roots_ntt(const GFT locator_poly[], size_t locator_poly_len, GFT roots[], GFT root_location[], GFT root_inv[]) const {
        std::fill_n(&roots[locator_poly_len], ecc_len - locator_poly_len, GFT{0});
    ...
    inline size_t _norm_size_rev(GFT r[], size_t r_len) const {
        ...
        if constexpr (std::is_integral_v<GFT>) {
            ...
            size_t start = r_len;
            while (start < ecc_len && std::all_of(&r[start][0], &r[start + 1][0], [](auto v) { return v == 0; }))
                ++start;

            if (start == ecc_len) {
                start = 0;
                while (start < r_len && std::all_of(&r[start][0], &r[start + 1][0], [](auto v) { return v == 0; }))
                    ++start;
            }
...
template <size_t W>
void RSi16v<W>::repair(GFT block[], GFT temp1_ecc6[]) const {
    d->repair(
        reinterpret_cast<simd_map_t<W> *>(block),
